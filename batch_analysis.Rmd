...
volc(rd_cal27_newVold_par,'log2FoldChange','padj','-log10(FDR)','')
```
...
volc(rd_cal27_newVold_nsd1ko ,'log2FoldChange','padj','-log10(FDR)','')
```
...
volc(rd_cal27_oldVnew_nsdDko,'log2FoldChange','padj','-log10(FDR)','')
```

# Relative to parental {.tabset }

<b> Proportional changes in gene expression in NSD1KO & NSD.DKO between the two batches. However, when we use the cutoff for DEGs (padj < 0.05 & |log2FC| > 2), we observe greater number of DEGs in the old batch than in the new batch. </b>
