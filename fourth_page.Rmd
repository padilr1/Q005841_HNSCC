...
For NSD1KO, P53 pathway is also upregulated. It should also be upregulated for NSD.DKO, but it is not.
<p>&nbsp;</p>
For all conditions, Kras signaling down should be upregulated while Kras signaling up should be downregulated.
<p>&nbsp;</p>
In summary, still unclear while ssGSEA differs from the differential gene expression GSEA.
...
pps <- c(ps$`C5.GO:BP`['GOBP_NEURON_DIFFERENTIATION'],
         ps$H['HALLMARK_OXIDATIVE_PHOSPHORYLATION'],
         ps$H['HALLMARK_KRAS_SIGNALING_DN'],
         ps$H['HALLMARK_KRAS_SIGNALING_UP'],
         ps$H['HALLMARK_P53_PATHWAY'])
...
```{r echo=FALSE, message=FALSE, warning=FALSE,out.width='100%',fig.width=6,fig.height=9,dev='svg'}
r %>% as.data.frame() %>% 
...
For NSD1KO, 2 samples are upregulated for oxidative phosphrylation while 1 sample is downregulated. The two samples with upregulated NSD1KO also have upregulated P53 pathway. It's still unclear what to make of the results. 

```{r include=FALSE}
#To avoid computing strict overlaps, we can compare things at a pathway level by computing ssGSEA scores for some previously enriched pathways, such as those related to metabolism or neurogenesis. The only consistent trend for the two pathways checked appears to be the inactivation of oxidative phosphorylation (Warburg effect?) upon the ablation of NSD1 or both NSD1 and NSD2, but not when K27M-KO is already removed, though this could also be a batch effect.
load("~/Documents/Q005841_HNSCC/data/dds_detroit_newbatchOnly.RData")
load("~/Documents/Q005841_HNSCC/data/mdat_Detroit_newbatch_only.RData")
vsd <- vst(dds, blind = T)
...
r %>% as.data.frame() %>% 
  rownames_to_column('pathway') %>%
  pivot_longer(-pathway, names_to = 'samp', values_to = 'score') %>%
  merge( rownames_to_column(rbind(cdatNew_hnscc_final[,'condition',drop=F]), 'samp')) %>%
  arrange(condition, samp, pathway) %>%
  mutate(samp = fct_inorder(samp)) %>%
  ggplot(aes(x = samp, y = score, color = condition)) +
  geom_point() +
...
<b> Analysis of overlapping genes between NSD1KO v parental & NSD2KO v parental. 2 reps per condition, including the parental (old and new batch parentals combined). We observe similar results as when we had 3 replicates for NSD2KO. </b>
...
load("~/Documents/Q005841_HNSCC/data/new_hnscc_cal27_only2NSDKO_reps.RData")
...
<b> One replicate for each of the condition compared to parental. </b>
<p>&nbsp;</p>
...
#need to perform shrinking of the log2 fold changes to account for Low counts & High dispersion values
#As with the shrinkage of dispersion estimates, LFC shrinkage uses information from all genes to generate more accurate estimates. Specifically, the distribution of LFC estimates for all genes is used (as a prior) to shrink the LFC estimates of genes with little information or high dispersion toward more likely (lower) LFC estimates.
load(file="~/Documents/Q005841_HNSCC/data/oldNewHNSCC_dds_V3_splitNSD1KO.RData")
res_shrunken_cal27_nsd2oe_v_par <- get_res_df_shrunken(dds = dds,group = "condition",sampA = "NSD1KO.OE.NSD2",ref_sampB = "Parental")
res_shrunken_cal27_nsd1koR1_v_par <- get_res_df_shrunken(dds,"condition","NSD1KO_R1","Parental")
res_shrunken_cal27_nsd1koR2_v_par <- get_res_df_shrunken(dds,"condition","NSD1KO_R2","Parental")
#took the same genes in both condition
get_dfs_with_overlapping_genes<- function(type,rd1,rd2,rd3,ttl1,ttl2,ttl3){
  if(type == "up"){
    #get overlapping genes
    gene_overlap <- base::intersect(base::intersect(rd1$ensembl_gene_id,rd2$ensembl_gene_id),rd3$ensembl_gene_id)
    #get DGE df for sample 1
    rd1_df <- rd1[rd1$ensembl_gene_id %in% gene_overlap,]
    #get DGE df for sample 2
    rd2_df <- rd2[rd2$ensembl_gene_id %in% gene_overlap,]
    #get DGE df for sample 3
    rd3_df <- rd3[rd3$ensembl_gene_id %in% gene_overlap,]
    #get upregulated df
    rd1_df <- rd1_df[rd1_df$log2FoldChange > 0,] %>%
      dplyr::select(log2FoldChange)
    rd2_df <- rd2_df[rd2_df$log2FoldChange > 0,] %>%
      dplyr::select(log2FoldChange)
    rd3_df <- rd3_df[rd3_df$log2FoldChange > 0,] %>%
      dplyr::select(log2FoldChange)
    #name log2fc
    rd1_df$condition <- ttl1
    rd2_df$condition <- ttl2
    rd3_df$condition <- ttl3
    #merge dfs
    combined <- rbind(rd1_df,rd2_df,rd3_df)
    return(combined)
  }
  if(type == "down"){
    #get overlapping genes
    gene_overlap <- base::intersect(base::intersect(rd1$ensembl_gene_id,rd2$ensembl_gene_id),rd2$ensembl_gene_id)
    #get DGE df for sample 1
    rd1_df <- rd1[rd1$ensembl_gene_id %in% gene_overlap,]
    #get DGE df for sample 2
    rd2_df <- rd2[rd2$ensembl_gene_id %in% gene_overlap,]
...
combined_nsd1ko_nsd2oe_up_shrunken <- get_dfs_with_overlapping_genes("up",res_shrunken_cal27_nsd1koR1_v_par,res_shrunken_cal27_nsd1koR2_v_par,res_shrunken_cal27_nsd2oe_v_par,"NSD1KO_R1","NSD1KO_R2","NSD1KO.NSD2OE") %>% na.omit()
#colnames(combined_nsd1ko_nsd2ko_up_shrunken) <- c("log2FoldChange_up","condition")
combined_nsd1ko_nsd2oe_down_shrunken <- get_dfs_with_overlapping_genes("down",res_shrunken_cal27_nsd1koR1_v_par,res_shrunken_cal27_nsd1koR2_v_par,res_shrunken_cal27_nsd2oe_v_par,"NSD1KO_R1","NSD1KO_R2","NSD1KO.NSD2OE") %>% na.omit()
#colnames(combined_nsd1ko_nsd2ko_down_shrunken) <- c("log2FoldChange_down","condition")
...
p_up <- ggplot(data = combined_nsd1ko_nsd2oe_up_shrunken,aes(x = condition,y = log2FoldChange),position = "stack",color = "darkgray", size = 2) +
  ...
  theme(plot.title = element_text(hjust = 0.5,vjust = 2.5),
        ...
        panel.border = element_blank(),
        panel.background = element_blank()) +
  geom_text(x=1,y=1.5,label=paste0(summary_up[summary_up$condition=="NSD1KO_R1","log2FoldChange"],"±",summary_up[summary_up$condition=="NSD1KO_R1","se"]),size=3) +
  geom_text(x=2,y=1.5,label=paste0(summary_up[summary_up$condition=="NSD1KO_R2","log2FoldChange"],"±",summary_up[summary_up$condition=="NSD1KO_R2","se"]),size=3) +
  geom_text(x=3,y=1.5,label=paste0(summary_up[summary_up$condition=="NSD1KO.NSD2OE","log2FoldChange"],"±",summary_up[summary_up$condition=="NSD1KO.NSD2OE","se"]),size=3) +
  coord_cartesian(clip = "off")
#
p_down <- ggplot(data = combined_nsd1ko_nsd2oe_down_shrunken,aes(x = condition,y = log2FoldChange),position = position_jitter(0.2),color = "darkgray", size = 2) +
  ...
  theme(plot.title = element_text(hjust = 0.5,vjust = 2.5),
        ...
        panel.border = element_blank(),
        panel.background = element_blank()) +
  geom_text(x=1,y=-1.5,label=paste0(summary_down[summary_down$condition=="NSD1KO_R1","log2FoldChange"],"±",summary_down[summary_down$condition=="NSD1KO_R1","se"]),size=3) +
  geom_text(x=2,y=-1.5,label=paste0(summary_down[summary_down$condition=="NSD1KO_R2","log2FoldChange"],"±",summary_down[summary_down$condition=="NSD1KO_R2","se"]),size=3) +
  geom_text(x=3,y=-1.5,label=paste0(summary_down[summary_down$condition=="NSD1KO.NSD2OE","log2FoldChange"],"±",summary_down[summary_down$condition=="NSD1KO.NSD2OE","se"]),size=3)
#get stats
stat.test_up <- compare_means(log2FoldChange ~ condition, data = combined_nsd1ko_nsd2oe_up_shrunken,paired = FALSE,p.adjust.method = "fdr", method = 't.test',method.args = list(alternative = "equal")) %>% dplyr::select(-".y.") %>% mutate(y.position = c(11,13,15))
stat.test_down <- compare_means(log2FoldChange ~ condition, data = combined_nsd1ko_nsd2oe_down_shrunken,paired = FALSE,p.adjust.method = "fdr", method = 't.test',method.args = list(alternative = "equal")) %>% dplyr::select(-".y.") %>% mutate(y.position = c(-12,-16,-20))
#make final graphic object
p_up_final <- p_up + stat_pvalue_manual(stat.test_up, label = "p.adj = {p.adj}",size = 3)
p_down_final <- p_down + stat_pvalue_manual(stat.test_down,label = "p.adj = {p.adj}",size = 3,tip.length = 0)
plot_grid(p_up_final,p_down_final,ncol = 1)
```

## Scatterplot

Fairly weak correlation in gene expression changes between NSD1KO and NSD1KO.NSD2OE. The correlation between NSD1KO replicates was expected to be much higher. 
...
rd_cal27_nsd1koR1_v_par<- get_dge_df(dds = dds,group = "condition",sampA = "NSD1KO_R1",ref_sampB = "Parental")
rd_cal27_nsd1koR2_v_par <- get_dge_df(dds = dds,group = "condition",sampA = "NSD1KO_R2",ref_sampB = "Parental")
nsd2oe_nsd1koR1_sctr_plt <- get_intersect_sctr_plt("all",rd_cal27_nsd2oe_v_par,rd_cal27_nsd1koR1_v_par,xlab = "log2FC NSD1KO.NSD2OE v Parental",ylab = "log2FC NSD1KO_R1 v Parental")
nsd2oe_nsd1koR2_sctr_plt <- get_intersect_sctr_plt("all",rd_cal27_nsd2oe_v_par,rd_cal27_nsd1koR2_v_par,xlab = "log2FC NSD1KO.NSD2OE v Parental",ylab = "log2FC NSD1KO_R2 v Parental")
nsd1ko_r1_r2 <- get_intersect_sctr_plt("all",rd_cal27_nsd1koR1_v_par,rd_cal27_nsd1koR2_v_par,xlab = "log2FC NSD1KO_R1 v Parental",ylab = "log2FC NSD1KO_R2 v Parental")
#arrange into 3-grid plot
ggarrange(nsd2oe_nsd1koR1_sctr_plt$p,nsd2oe_nsd1koR2_sctr_plt$p,nsd1ko_r1_r2$p)
```
<p>&nbsp;</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
...
* NSD1KO has large effect on transcriptome
* NSD2KO has considerably smaller effect
* NSD.DKO behaves similar to NSD1KO
* NSD.TKO behaves differently to the other conditions
* NSD2OE seems to rescue NSD1KO 

# Future Directions 

* Get more replicates of NSD2OE in Cal27 and in other cell lines. Might be a good idea to also increase the number of parental replicates.
* Reproduce comparison of NSD1KO v NSD2KO in other cell lines to confirm that NSD1KO does indeed have a greater effect on the transcriptome than NSD2KO. Suggest having at least 2 replicates for parental.
* What happens if we do a double knockout of NSD2 and NSD3 only? Do we observe similar changes in the transcriptome to NSD2KO? How about NSD1 and NSD3 double knockout?
