...
We find more upregulated genes in NSD2.KO than downregulated ones. However, overrepresentation of similar pathways are found in both upregulated and downregulated genes, such as cell periphery and development process. However, downregulated genes are also enriched in pathways in cancer & calcium signaling. 
...
get_euler_graph <- function(ttl1,ttl2,ttl3,x1,x2,x3,logfc_cutoff){
x <- list(c1 = x1$ensembl_gene_id[x1$padj < .05 & abs(x1$log2FoldChange) > logfc_cutoff],c2 = x2$ensembl_gene_id[x2$padj < .05 & abs(x2$log2FoldChange) > logfc_cutoff],c3 = x3$ensembl_gene_id[x3$padj < .05 & abs(x3$log2FoldChange) > logfc_cutoff])
...
Essentially we recapitulate what we see from PCA and volcano plots, in that NSD.1KO is not so different from NSD.DKO. NSD.2KO seems to be very different to NSD.TKO?
...
NSD1.KO and NSD.DKO had few DEGs relative to all others and few overrepresented pathways. On the other hand, NSD2.KO and NSD.TKO had a greater number of DEGs and overrepresented pathways. 
...
Comparing NSD1.KO to K36M-OE, cell periphery is an enriched pathway, similar to when comparing NSD1.KO to NSD2.KO. 
<p>&nbsp;</p>
Comparing NSD.DKO to K36M-OE, overrepresentation of genes related to chromosome. 
...
Comparing downregulated NSD1.KO, NSD2.KO or NSD.DKO to K36M-OE, similar overrepresented pathways are found as when comparing NSD1.KO to NSD2.KO. Example: cellular structure development, cellular development
...
NSD2.KO seems to have different enriched pathways in the downregulated genes: cholesterol homeostasis. NSD1KO and NSD.DKO compared to K36M-OE seems to have similarly enriched pathways.
...
<p>&nbsp;</p>
86 genes differentially expressed when we perturb NSD1 compared to K36M-OE
94 genes in common when we perturb NSD2 compared to K36M-OE
...
#
get_euler_graph("K36M.OE","NSD1.KO","NSD2.KO",k36m.OE_v_nsd1ko$x1,k36m.OE_v_nsd1ko$x2,k36m.OE_v_nsd2ko$x2,0.05)
```
...
det_k36m.OE_v_nsd1ko <- overlap_analysis(dds = dds,grouping = "group",cond1 = "Cal27OE.K36M",cond2 = "Cal27NSD1.KO",baseCond = "Cal27OE.K36WT",fc_cutoff = 0.5)
#
